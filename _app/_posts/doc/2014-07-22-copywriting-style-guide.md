...
* 英文与非标点的中文之间需要有一个空格，如「使用 AVOS Cloud 开发移动应用」而不是「使用AVOS Cloud开发移动应用」。[^1]
...
* 字体和字号的一致：在富文本格式文档中，特别是 HTML 邮件中，常有人因为复制粘贴而导致同一层次的文本字体和字号不一致。这给人不专业的感觉，请避免。
* App 是 application 的缩写，是一个普通名词而不是缩写，所以不应该用全大写的 APP。和其他词一样，大小写规则取决于是否处于句首或标题位置，如 App Store。国内之所以很多地方用全大写，是因为很多人把这个词发音做「A 批批」，我们应该有意识地避免。大多数情况下应该使用中文「应用」以避免这样的问题。
